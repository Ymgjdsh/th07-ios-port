...
for relative, markers in online_markers.items():
    ...
    for marker in markers:
        if marker not in source:
            print("error: build 37 Online marker missing:", relative, marker)
            failed = True
...
if failed:
    sys.exit(2)
print("ok: Build 43 host/guest local touch prediction, EX/PH touch controls, cheat code and title texture markers")
