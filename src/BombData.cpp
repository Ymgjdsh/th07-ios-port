...
void BombData::SpawnBombInvulnEffect(Player *player)
{
    ...
    Effect *effect = g_EffectManager.SpawnEffect(
        25, &player->positionCenter, GetPlayerEffectSlot(player, 0), 1, 0xffffffff);
...
void BombData::BombReimuACalc(Player *player)
{
    ...
    {
        g_Gui.EndPlayerSpellcard(player->initParam);
    ...
    {
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1185), "霊符「夢想封印　散」");
        ...
        bombInfo->bombTimer.GetCurrent() % 6 == 0)
    {
        ...
        for (j = 0; j < 4; j++, vm++)
        {
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, j + ANM_SCRIPT_REIMU_A_BOMB_ARRAY));
        }
...
void BombData::BombReimuACalcFocus(Player *player)
{
    ...
    if (bombInfo->bombTimer.HasTicked() && bombInfo->bombTimer == 0)
    {
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1185), "霊符「夢想封印　集」");
...
void BombData::BombReimuBCalc(Player *player)
{
    ...
    {
        g_ItemManager.RemoveAllItems();
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1185), "夢符「封魔陣」");
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, i + ANM_SCRIPT_REIMU_B_BOMB_ARRAY));
        }
...
void BombData::BombReimuBCalcFocus(Player *player)
{
    ...
    {
        g_ItemManager.RemoveAllItems();
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1185), "夢符「二重結界」");
        ...
        {
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, i + ANM_SCRIPT_REIMU_B_FOCUS_BOMB_ARRAY));
        }
        g_SoundPlayer.PlaySoundByIdx(SOUND_BOMB_REIMARI, 0);
        BombEffects::RegisterChain(1, 60, 2, 6, 0);
        player->bombStartPos = player->positionCenter;
...
void BombData::BombMarisaACalc(Player *player)
{
    ...
    {
        g_ItemManager.RemoveAllItems();
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1187),
                                   "魔符「スターダストレヴァリエ」");
        ...
        for (i = 0; i < 8; i++, vm++)
        {
            vm = player->bombInfo.subInfo[i].vms;
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, i % 3 + ANM_SCRIPT_MARISA_A_BOMB_ARRAY));
...
void BombData::BombMarisaACalcFocus(Player *player)
{
    ...
    {
        g_ItemManager.RemoveAllItems();
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1186), "魔符「ミルキーウェイ」");
...
void BombData::BombMarisaBCalc(Player *player)
{
    ...
    if (player->bombInfo.bombTimer.HasTicked() && player->bombInfo.bombTimer == 0)
    {
        g_ItemManager.RemoveAllItems();
        player->bombStartPos = player->positionCenter;
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1185),
                                   "恋符「ノンディレクショナルレーザー」");
        ...
        {
            g_AnmManager->ExecuteAnmIdx(
                subInfo->vms, GetPlayerAnmScript(player, i + ANM_SCRIPT_MARISA_B_BOMB_ARRAY));
            subInfo->prevBombRegionPositions = subInfo->bombRegionPositions =
                player->positionCenter;
...
void BombData::BombMarisaBCalcFocus(Player *player)
{
    ...
    {
        g_ItemManager.RemoveAllItems();
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1186), "恋符「マスタースパーク」");
        ...
        {
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, i + ANM_SCRIPT_MARISA_B_FOCUS_BOMB_ARRAY));
            player->bombInfo.subInfo[i].bombRegionPositions = player->positionCenter;
        }
        g_SoundPlayer.PlaySoundByIdx(SOUND_BOMB_SAKUMARI, 0);
                ...
                if (player->bombInfo.bombTimer <= 120 && spawnsRemaining != 0)
                {
                    ...
                    g_AnmManager->ExecuteAnmIdx(
                        vm, GetPlayerAnmScript(
                                player, ANM_SCRIPT_SAKUYA_A_BOMB_ARRAY + (i & 1)));
...
void BombData::BombSakuyaACalcFocus(Player *player)
{
    ...
    {
        g_ItemManager.RemoveAllItems();
        g_Gui.ShowBombNamePortrait(player->initParam, GetPlayerAnmScript(player, 1185), "幻符「殺人ドール」");
    ...
    if (player->bombInfo.bombTimer >= 20 && player->bombInfo.bombTimer < 116)
    {
        ...
        for (i = 0; i < 96; i++, subInfo++)
        {
            ...
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(
                        player, (i & 1) + ANM_SCRIPT_SAKUYA_A_FOCUS_BOMB_ARRAY));
...
void BombData::BombSakuyaBCalc(Player *player)
{
    ...
    if (player->GetBombTimer()->HasTickedAndIsEq(30))
    {
        ...
        for (i = 0; i < 4; i++, subInfo++)
        {
            ...
            vm = subInfo->vms;
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, i + ANM_SCRIPT_SAKUYA_B_BOMB_ARRAY));
...
void BombData::BombSakuyaBCalcFocus(Player *player)
{
    ...
    if (player->GetBombTimer()->HasTickedAndIsEq(0))
    {
        ...
        for (i = 0; i < 2; i++, subInfo++)
        {
            ...
            vm = subInfo->vms;
            g_AnmManager->ExecuteAnmIdx(
                vm, GetPlayerAnmScript(player, i + ANM_SCRIPT_SAKUYA_B_FOCUS_BOMB_ARRAY));
            subInfo->prevBombRegionPositions = subInfo->bombRegionPositions =
                player->positionCenter;
            for (j = 31; j >= 0; j--)
            {
